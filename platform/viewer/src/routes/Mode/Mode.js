...
export default function ModeRoute({
  ...
  useEffect(() => {
    ...
    if (studyInstanceUIDs && studyInstanceUIDs.length && studyInstanceUIDs[0] !== undefined){
      retrieveLayoutData();
    }
    return () => {
      layoutTemplateData.current = null;
  ...
  useEffect(() => {
    ...
    if (userPreferredHotkeys && userPreferredHotkeys.length){
