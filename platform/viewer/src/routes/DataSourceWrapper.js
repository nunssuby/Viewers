...
function DataSourceWrapper(props) {
  ...
  if (!dataSourceName && window.config.defaultDataSourceName) {
    ...
    dataSourceName = webApiDataSources
      .map(ds => ds.name)
      .find(it => {
        const dataSources = extensionManager.getDataSources(it);
        return dataSources && dataSources[0] !== undefined;});
  }
  // const dataSource = extensionManager.getDataSources(dataSourceName)?.[0];
  const dataSource = extensionManager.getDataSources(dataSourceName);
  const firstItem = dataSource ? dataSource[0] : undefined;
  if (!dataSource) {
